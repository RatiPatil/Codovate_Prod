const express = require('express');
const router = express.Router();
const { db } = require('../config/firebase');
const { body, validationResult } = require('express-validator');

const superAdminOnly = (req, res, next) => {
  if (req.user.role !== 'super_admin' && req.user.role !== 'admin') {
    return res.status(403).json({ message: 'Super Admin access required.' });
  }
  next();
};

// GET all certificates
router.get('/', superAdminOnly, async (req, res) => {
  try {
    const snapshot = await db.collection('certificates').get();
    const certs = [];
    snapshot.forEach(doc => {
      certs.push({ id: doc.id, ...doc.data() });
    });
    res.json(certs);
  } catch (error) {
    res.status(500).json({ message: 'Server error' });
  }
});

// POST new certificate
router.post('/', superAdminOnly, [
  body('title').notEmpty().withMessage('Title is required'),
  body('student_id').notEmpty().withMessage('Student ID is required'),
  body('issue_date').notEmpty().withMessage('Issue Date is required'),
  body('status').isIn(['valid', 'revoked']).withMessage('Invalid status')
], async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });

  try {
    const { title, student_id, issue_date, issuer, credential_id, credential_url, status } = req.body;
    const newDocRef = db.collection('certificates').doc();
    const newCert = {
      id: newDocRef.id,
      title,
      student_id,
      issue_date,
      issuer: issuer || 'Codovate',
      credential_id: credential_id || newDocRef.id.substring(0, 10).toUpperCase(),
      credential_url: credential_url || '',
      status,
      created_at: new Date()
    };
    await newDocRef.set(newCert);
    res.status(201).json(newCert);
  } catch (error) {
    res.status(500).json({ message: 'Server error' });
  }
});

// PUT update certificate
router.put('/:id', superAdminOnly, [
  body('title').optional().notEmpty(),
  body('status').optional().isIn(['valid', 'revoked'])
], async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });

  try {
    const docRef = db.collection('certificates').doc(req.params.id);
    const doc = await docRef.get();
    if (!doc.exists) return res.status(404).json({ message: 'Certificate not found' });

    const updateData = { ...req.body };
    await docRef.update(updateData);
    const updated = await docRef.get();
    res.json(updated.data());
  } catch (error) {
    res.status(500).json({ message: 'Server error' });
  }
});

// DELETE (Soft delete) certificate
router.delete('/:id', superAdminOnly, async (req, res) => {
  try {
    const docRef = db.collection('certificates').doc(req.params.id);
    const doc = await docRef.get();
    if (!doc.exists) return res.status(404).json({ message: 'Certificate not found' });

    await docRef.update({ status: 'revoked' });
    res.json({ message: 'Certificate revoked successfully' });
  } catch (error) {
    res.status(500).json({ message: 'Server error' });
  }
});

module.exports = router;
