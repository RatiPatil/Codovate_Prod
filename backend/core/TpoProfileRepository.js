...
class TpoProfileRepository extends FirestoreRepository {
  constructor() { super('tpo_profiles'); }
  ...
  async create(data, userContext) {
    ...
    if (data.employeeId) {
      ...
      if (!existing.empty && existing.docs[0].id !== userId) {
        throw new AppError(`TPO Staff with Employee ID ${data.employeeId} already exists.`, 409);
      }
    }
...
module.exports = new TpoProfileRepository();
