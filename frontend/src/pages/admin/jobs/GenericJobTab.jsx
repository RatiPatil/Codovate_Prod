...
import { Edit, Archive, PlayCircle, StopCircle, Trash, Plus, Download } from 'lucide-react';
import { Button } from '../../../../components/admin/ui/Button';

export const GenericJobTab = ({ 
  title, 
  apiClient, 
  columns, 
  formSchema, 
  ...
  const handleAction = async (action, id) => {
    try {
      if (action === 'PUBLISH') {
        if (!window.confirm('Publish this job? It will be visible to students.')) return;
        await apiClient.publish(id);
      } else if (action === 'CLOSE') {
        if (!window.confirm('Close this job? No more applications will be accepted.')) return;
        await apiClient.close(id);
  ...
  const internalColumns = [
    ...
    { 
      ...
      key: 'recordStatus', 
      render: (val) => {
        const variants = { DRAFT: 'default', PUBLISHED: 'success', CLOSED: 'warning', ARCHIVED: 'danger' };
      ...
      render: (_, row) => (
        <div className="flex items-center gap-2">
          ...
          {row.recordStatus === 'DRAFT' && (
            <button onClick={() => handleAction('PUBLISH', row.id)} className="p-1 text-gray-500 hover:text-green-600" title="Publish">
              <PlayCircle className="w-4 h-4" />
            </button>
          )}

          {row.recordStatus === 'PUBLISHED' && (
            <button onClick={() => handleAction('CLOSE', row.id)} className="p-1 text-gray-500 hover:text-orange-600" title="Close">
