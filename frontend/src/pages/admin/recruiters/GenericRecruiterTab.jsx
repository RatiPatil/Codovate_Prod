...
export const GenericRecruiterTab = ({ 
  title, 
  apiClient, 
  columns, 
  formSchema, 
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      {isFormOpen && (
        <Modal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} title={editingRecord ? `Edit ${title}` : `Register ${title}`}>
          <div className="py-4 h-[70vh] overflow-y-auto px-2">
            <GenericForm 
              ...
              submitLabel={editingRecord ? 'Save Changes' : `Register ${title}`}
            />
          </div>
        </Modal>
      )}
