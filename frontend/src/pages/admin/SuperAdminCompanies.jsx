...
const SuperAdminCompanies = () => {
  ...
  const fetchCompanies = async () => {
    try {
      ...
      setCompanies(response.data.filter(u => u.role === 'company_admin'));
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="p-8">
      <AdminDataTable title="Manage Companies" data={companies} columns={columns} loading={loading} searchPlaceholder="Search companies..." />
    </div>
  );
};
