...
export const GenericPlacementTab = ({ 
  title, 
  apiClient, 
  columns, 
  formSchema, 
  ...
  const internalColumns = [
    ...
    { 
      ...
      key: 'recordStatus', 
      render: (val) => {
        const variants = { ACTIVE: 'success', Upcoming: 'indigo', Active: 'success', Completed: 'default', ARCHIVED: 'warning', SUSPENDED: 'danger' };
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex items-center gap-2">
          ...
          {(!row.recordStatus || row.recordStatus === 'ACTIVE' || row.recordStatus === 'Upcoming' || row.recordStatus === 'Active') ? (
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      {isFormOpen && (
        <Modal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} title={editingRecord ? `Edit ${title}` : `New ${title}`}>
          <div className="py-4 h-[60vh] overflow-y-auto px-2">
            <GenericForm 
              ...
              submitLabel={editingRecord ? 'Save Changes' : `Save ${title}`}
            />
          </div>
        </Modal>
      )}
