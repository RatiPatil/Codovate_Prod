...
const CompanyAdminCandidates = () => {
  ...
  const fetchCandidates = async () => {
    try {
      ...
      // Filter for those in interview (Under Review) stage or selected
      setCandidates(response.data.filter(app => app.status === 'Under Review' || app.status === 'Selected'));
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
...
export default CompanyAdminCandidates;
