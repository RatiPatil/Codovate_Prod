      ...
      <div ref={formRef} className="w-full max-w-lg relative z-10 space-y-5">
        ...
        <div className="text-center">
          ...
          <p className="text-gray-500 text-xs">
            Want to join as a partner? <a href="mailto:partners@codovate.in" className="text-amber-500 hover:underline">Contact us</a>
          </p>
        </div>
        ...
        <div className="bg-[#080812]/90 backdrop-blur-xl border border-white/8 rounded-2xl shadow-2xl relative overflow-hidden">
          ...
          <div className="p-7 space-y-5">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-[9px] font-black text-gray-500 uppercase tracking-widest mb-2">Recruiter Email</label>
                <input
                  type="email"
                  value={form.email}
                  ...
                  required
                  className="w-full bg-black/40 border border-white/8 rounded-xl py-3.5 px-4 text-white text-sm placeholder-gray-600 focus:border-amber-500/60 focus:outline-none transition-all"
                />
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className={`w-full bg-amber-600 hover:bg-amber-500 text-white font-bold py-4 rounded-xl text-sm transition-all mt-6 shadow-[0_10px_30px_rgba(245,158,11,0.2)] ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
              >
                {loading ? 'Authenticating...' : 'Sign in to Codovate HQ'}
              </button>
            </form>
          </div>
        </div>
